...
@section('main')
    ...
    <div class="container-fluid">
        ...
        <div class="card card-default" id="news-form">
            <div class="card-header">
                @if($edit)
                    {{route('bourse.show' ,['url'=>$edit->url  ])}}
                @else
                    فرم ثبت خبر
                @endif
            </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <div class="input-icon">
                                    ...
                                    <textarea  class="form-control"  name="text" id="text">{{$edit ? $edit->text : ''}}</textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row" style="padding:15px">
                        ...
                        <div class="col-md-4 pull-left">
                            <button type="submit" class="btn btn-secondary" id="save-form-btn" onclick="save_news()">
                                <span class="nospiner"> {{$edit ? 'ویرایش' : 'ثبت' }} خبر </span>
    ...
    <script>
        ...
        function save_news()
        {
            ...
            $.post('{{route('cp.bourse.save' , ['id'=>$edit ? $edit->id : ''])}}' , csrf(obj)  , function(data){
                unloadspiner(btn);
                data = $.parseJSON(data);
                if(data.stat == 'ok')
                {
                    ok('با موفقیت انجام شد ');
                    document.location = '{{route('cp.bourse')}}' ;
                }
                else
                {
                    er(data.msg);
